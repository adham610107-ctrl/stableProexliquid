...
function startDashClock() {
    function tick() {
        ...
        // Last online: sessiya boshidan qancha vaqt o'tdi
        const diffSec = Math.floor((Date.now() - sessionStartTime) / 1000);
        const lo = document.getElementById('dash-last-online');
        if (lo) {
            if (diffSec < 60)       lo.innerText = "So'nggi faollik: hozirgina";
            else if (diffSec < 3600) lo.innerText = `So'nggi faollik: ${Math.floor(diffSec/60)} daqiqa oldin`;
            else                    lo.innerText = `So'nggi faollik: ${Math.floor(diffSec/3600)} soat oldin`;
        }
    }
    tick();
...
async function handleLogin() {
    ...
    sessionStartTime = Date.now(); // Last online reset
...
function startTestSession(){
    ...
    _applyLiquidLevels();stopWaterLoop();initGyro();
}
...
function checkAns(qIdx,optIdx,event){
    ...
    if(isCorrect){
        ...
        // TO'G'RI: faqat xato variantlarni suvga tashlash
        setTimeout(()=>{
            for(let i=0;i<allBtns.length;i++){
                if(allBtns[i]!==clickedBtn){
                    dropToWater(allBtns[i], i * 60);
                }
            }
        }, 300);
    } else {
        ...
        // XATO: BARCHA variantlarni suvga tashlash (to'g'ri javob ham ko'rinmasin)
        setTimeout(()=>{
            for(let i=0;i<allBtns.length;i++){
                dropToWater(allBtns[i], i * 80);
            }
        }, 250);
    }
...
// Qonunlar: inertiya, suzuvchi kuch, siltanish, so'nish
// ============================================================
let gyroRafId = null;
